...
def get_ellipse_points(semi_major, semi_minor, alpha):
    ...
    edge1 = get_edge_point(aa, bb, cc, ellipse_centre_x, ellipse_centre_y, np.tan(-np.pi / 6.0))
    edge2 = get_edge_point(aa, bb, cc, ellipse_centre_x, ellipse_centre_y, np.tan(np.pi / 6.0))
    line_fd1 = get_line_through_points(focus, edge1)
    line_fd2 = get_line_through_points(focus, edge2)
    detector_point1 = get_intersect_two_lines(line_fd1, np.array([1, 0, -0.5]))
    detector_point2 = get_intersect_two_lines(line_fd2, np.array([1, 0, -0.5]))
        ...
        while True:
            ...
            if 2 > semi_major_new > semi_minor_new > 0 and -np.pi / 4. > alpha_new > -np.pi / 2.:
                focus_new, edge1_new, edge2_new, ellipse_centre_x_new, ellipse_centre_y_new, detector_point1_new, detector_point2_new = get_ellipse_points(
                    semi_major_new, semi_minor_new, alpha_new)
                if detector_point2_new[1] < -1 < detector_point1_new[1] < focus_new[1] - 0.1 < -0.5 - 0.1 and edge1_new[
                    0] > 1:
                    break
...
plt.text(0.7, -2.5,
         'Semi-major axes {:5.3f}m\nSemi-minor axes {:5.3f}m\nRotating Angle={:5.3f}°'.format(semi_major, semi_minor,
                                                                                              np.rad2deg(alpha)))
plt.savefig('Mushroom_Geometry.pdf')
