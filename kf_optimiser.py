...
POINT_START = instr.cryo_radius * np.array([1, np.tan(instr.angle_plus_rad)])
...
def get_endpoint(sf, angle):
    ...
    distance_sa = geo.points_distance(point1=POINT_SAMPLE, point2=POINT_START)
    distance_af = geo.points_distance(point1=POINT_START, point2=point_focus)
    ellipse_semimajor = (distance_sa + distance_af) / 2.0

    x0, y0 = point_focus
    a = ellipse_semimajor
    x = (4 * a ** 2 - x0 ** 2 - y0 ** 2) / (
            4 * a / np.cos(instr.angle_minus_rad) - 2 * (x0 + y0 * np.tan(instr.angle_minus_rad)))
    point_end = x * np.array([1, np.tan(instr.angle_minus_rad)])
    return point_end
...
def focus2kf(sf, angle):
    ...
    distance_af = geo.points_distance(point1=POINT_START, point2=point_focus)
    ellipse_semimajor = (distance_sa + distance_af) / 2.0
    ellipse_lin_eccen = geo.points_distance(point1=POINT_SAMPLE, point2=point_focus) / 2.0
...
def farest_detector(sf, angle):
    ...
    line_af = geo.points_to_line(point1=POINT_START, point2=point_focus)
    point_detector = geo.lines_intersect(line1=line_af, line2=detector_line)
    return point_detector


distance_sf = np.linspace(instr.cryo_radius, 3, num=PLOT_POINTS)
angle_focus = np.deg2rad(np.linspace(instr.angle_minus_deg - 1, -90, num=PLOT_POINTS))
