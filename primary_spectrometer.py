...
def plot_resolution_polarangles(geo_ctx: GeometryContext, polar_angles, all_dqx_m, all_dqy_m, all_dqz_m, all_kf):
    ...
    # plt.tight_layout()
    # plt.subplot(133)
    plt.plot(polar_angles, all_dqx_m * 1e-10, color="royalblue")
    plt.plot(polar_angles, all_dqy_m * 1e-10, color="red")
    plt.plot(polar_angles, all_dqz_m * 1e-10, color="gold")
    # plt.plot(polar_angles, all_kf * 1e-10)
    plt.xlabel(r"Polar angle $\phi$ (degree)")
    plt.ylabel(r"$\Delta Q$ (Angstrom$^{-1}$ )")
    # plt.yscale('log')
    plt.grid()
    plt.legend(("x - horizontal", "y - vertical", "z - along kf"))
    # plt.title('z')
    plt.tight_layout()
